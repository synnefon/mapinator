...
// reprojected each frame with the same projection as the canvas overlays. ONE set covers everything — the
// whole per-country rank-size list (cityStats.assembleCities), placed once with the map — sized by population
// and zoom-gated by minLevel (capitals + big cities on the globe, the small-town tail revealed as you zoom
// in). Dots open the shared popup; the limb cull + projection are the Projector's.
...
/**
 * Manages the interactive city-marker layer. `setCities` holds the whole settlement set (rebuilt when the map
 * result changes). `update` reprojects + zoom-gates it each frame and opens the shared info popup on click.
 */
export class CityMarkers {
  private readonly frame: HTMLElement;
  private readonly popup: InfoPopup;
  private cities: Settlement[] = []; // the whole rank-size settlement set (LOD-gated by minLevel)
  ...
  /** Set the whole settlement set (called when the cached map result changes). */
  ...
  /** Reproject + reposition every marker. A marker shows only if its reveal level is reached, it faces the
   *  camera, and the layer is visible — capitals + big cities first, the small-town tail deeper on zoom. */
