...
// suitability-weighted habitable area — so POPULATION.GLOBAL_POPULATION_DENSITY is the single dial that
// scales a world's head count to land near that ~2.5/km² figure. The remaining POPULATION dials weight the
...
export type PopInputs = {
  ...
  // suitability-weighted habitable area GLOBAL_POPULATION_DENSITY turns into people. Assembled in countries.ts.
...
export function estimatePopulation(p: PopInputs): number {
  ...
  const density = POPULATION.GLOBAL_POPULATION_DENSITY.value * p.government.densityFactor * variation;
