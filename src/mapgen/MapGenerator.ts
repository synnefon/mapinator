...
export class MapGenerator {
  ...
  private genMoistures(baseMap: BaseMap, settings: MapSettings): Float32Array {
    ...
    const moistureContrast = settings.moistureContrast ?? DEFAULT_MOISTURE_CONTRAST;
    ...
    const fbm2w = (x: number, y: number) => {
      ...
      return Math.min(1, Math.max(0, 0.5 + FBM_WEIGHTS.n1 * n1 + FBM_WEIGHTS.n2 * n2));
    };
    ...
    for (let r = 0; r < numRegions; r++) {
      ...
      const wx = nx + (warpStrength / WARP_DIVISOR) * (fbm2w(kWarp * nx, kWarp * ny) - 0.5);
      const wy = ny + (warpStrength / WARP_DIVISOR) * (fbm2w(kWarp * ny, kWarp * nx) - 0.5);
      const m = fbm2w((kRip / RIP_DIVISOR) * wx, (kRip / RIP_DIVISOR) * wy);
